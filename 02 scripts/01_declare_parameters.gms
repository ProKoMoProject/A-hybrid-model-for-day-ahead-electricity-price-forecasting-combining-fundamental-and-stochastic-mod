set
    ...
    bm     minute control power blocks
...
*technology parameters
    ...
    af_before_2015(i)            availability of pp before 2015 (without outage data)
    af_nuc_before_2015_up(month,n)
...
*demand parameters
    ...
    demand_2DA(t,n)        2DA demand
    TwoDayAhead_KIT(t,n)    electricity demand forecast two day ahead [MWh per hour]
    TwoDAlastweek(t,n)    electricity demand forecast two day ahead [MWh per hour]
...
* Control Power
    ...
    MR_pos(n)    minute reserve - positive
    MR_neg(n)    minute reserve - negative
    max_share_scr(i)  maximum share of running capacity that can be used for secondary control provision due to ramping rates
...
$onecho > Import_general.txt
    ...
    par=af_before_2015           rng=nodes!AE3        rdim=1 cdim=0
    par=af_nuc_before_2015_up    rng=nodes!AH2        rdim=1 cdim=1
...
$load cap_up
$LOAD af_tech_general, af_nuc_before_2015_up , af_before_2015
...
$onecho > Import_yearly.txt
    ...
    par=TwoDayAhead_KIT     rng=Demand_2DA_KIT!B1       rdim=1 cdim=1
...
$LOAD demand, TwoDAlastweek, TwoDayAhead_KIT
$LOAD res_gen
$LOAD wvf
$LOAD ntc
$Load CP_up
...
* --------------- Subset Definitions    --------------------
    ...
    gas_CCGT(i)   = techup(i,'Tech_class_3')= 4 ;
    gas_OCGT(i)   = techup(i,'Tech_class_3')= 5 ;
    gas_gassteam(i)  = techup(i,'Tech_class_3')= 6  ;
    gas_CHP(i)    = techup(i,'Tech_class_3')= 4 or techup(i,'Tech_class_3')= 6  ;
...
* ------------   Loading Parameters     ---------------------
    ...
    demand_2DA(t,n) = TwoDayAhead_KIT(t,n) + TwoDAlastweek(t,n) ;
    ...
    cap_gas_CHP(n,t)  = sum(gas_CHP, cap(gas_CHP,n,t))   ;
...
%Before2015% af_tech_general(Thermal,n) = af_before_2015(Thermal) ;

    af_tech_t(i,n,t) =  af_tech_general(i,n);
%Before2015%     af_tech_t(Nuclear,'FR',t) =  sum( (month)$map_MT(month,t), af_nuc_before_2015_up(month,'FR') ) ;
...
%Before2015%  outages_up(t,n,'nuc') = 0 ;
    ...
    SR_neg('DE') = CP_up('Secondary_neg') ;
     ...
     max_share_scr(i) = (techup(i,'Ramp per min')/100) * 5 ;
    ...
    CHP_gen_gas_cluster(gas_CHP,n,t)$cap_gas_CHP(n,t)    =   CHP_gen_gas(n,t) * cap(gas_CHP,n,t) / cap_gas_CHP(n,t) ;
